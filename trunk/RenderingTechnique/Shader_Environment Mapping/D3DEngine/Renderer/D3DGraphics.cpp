...
void cD3DGraphics::InitD3D()
{
	...
	LoadVertexShader(L"EnviromentMapping.vs");
//	LoadPixelShader(L"phong.ps");
	SetupMatrialLight();
}
...
cCamera* cD3DGraphics::GetDefaultCamera()
{
	assert(m_pDefaultCamera!=NULL);
...
void cD3DGraphics::Update()
{		
	ClearDebugString();
	if(m_bUsingDefaultCamera)
