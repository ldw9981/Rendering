...
int uVertexShader::Load(char *filename, DWORD *pDeclaration, char *verprog)
{
	int er=0;
	...
	if(pError!=NULL)
	{
		...
		pError->Release();
		MessageBox(hWnd, error_text, "Chyba pri nacitavani vertex shader", MB_OK);
		delete [] error_text;
		hVertexShader = NULL;
		return 0;
	}
	...
	{
		MessageBox(hWnd, filename, "Nenajdeny subor pre vertex shader", MB_OK);
		return 0;
	}
	...
	pCode->Release();
	MessageBox(hWnd, filename, "Chyba pri nacitavani vertex shader", MB_OK);
	return 0;
}
...
int uPixelShader::LoadPixelShader(char *filename, char *pixelprog)
{
	int er=0;
	...
	if(pError!=NULL)
	{
		...
		pError->Release();
		MessageBox(hWnd, error_text, "Chyba pri nacitavani pixel shader", MB_OK);
		delete [] error_text;
		hPixelShader = NULL;
		return 0;
	}
	...
	{
		MessageBox(hWnd, filename, "Nenajdeny subor pre pixel shader", MB_OK);
		return 0;
	}
	...
	pCode->Release();
	MessageBox(hWnd, filename, "Chyba pri nacitavani pixel shader", MB_OK);
	return 0;
}
