...
wstring& cASEParser::GetWString()
{
	...
	::MultiByteToWideChar(CP_ACP,MB_PRECOMPOSED,m_TokenString,static_cast<int>(strlen(m_TokenString)),WideBuffer,256);
	temp=WideBuffer;
	return temp;
}
...
BOOL cASEParser::Load( LPCTSTR strFileName ,SceneGraphNode* pOutput)
{	
	BOOL bResult;
	...
	while(m_Token = GetToken(m_TokenString), m_Token != TOKEND_END)
	{
		...
		default:
			{					
				...
				::MultiByteToWideChar(CP_ACP,MB_PRECOMPOSED,m_TokenString,
					static_cast<int>(strlen(m_TokenString)),
					strToken,256);							
...
{	
	...
	VertexBuffer* pNewRscVertexBuffer=NULL;
	IndexBuffer* pNewRscIndexBuffer=NULL;
	...
	while (m_Token=GetToken(m_TokenString),m_Token!=TOKEND_BLOCK_END)
	{
		...
		case TOKENR_TM_ANIMATION:
			{			
				TransformAnm* pRscTransformAnm=GetTransformAnm(pNewSceneNode->GetLocalTM());
	...
	{
		pNewSceneNode->SetTotalVertex(static_cast<int>(tempArrayNormalVertex.size()));

		pNewRscVertexBuffer = m_SharedResourceMng.CreateVertexBuffer(
		...
		pNewSceneNode->SetVetextBuffer(pNewRscVertexBuffer);
	}
	...
	{
		pNewRscIndexBuffer = m_SharedResourceMng.CreateIndexBuffer(
		...
		pNewSceneNode->SetIndexBuffer(pNewRscIndexBuffer);
	}			
			...
			case TOKENR_MAP_DIFFUSE:
				{
					...
					while (m_Token=GetToken(m_TokenString),m_Token!=TOKEND_BLOCK_END)
					{
						...
						case TOKENR_BITMAP:
							...
							Texture* pRscTexture= m_SharedResourceMng.CreateTexture(strFileName.c_str());
							if(pRscTexture==NULL)
								TRACE1(L"MAP_DIFFUSE: %s 파일이없습니다.\n",strFileName.c_str());
							Matrial.SetTexture(pRscTexture);
							break;
						}		
					}
				}
							...
							case TOKENR_MAP_DIFFUSE:
								{
									...
									{			
										...
										case TOKENR_BITMAP:
											...
											if(pRscTexture==NULL)
												TRACE1(L"MAP_DIFFUSE: %s 파일이없습니다.\n",strFileName.c_str());
											SubMatrial.SetTexture(pRscTexture);
										break;
										}
									}									
								}
...
BOOL cASEParser::Parsing_CameraObject()
{
	...
	CameraNode* pNewSceneNode=new CameraNode;
	pNewSceneNode->SetRootNode(m_pSceneRoot);	
...
/*
	보간할때 localTM이 필요하다.
	TransformAnm을 리소스매니져에서 생성한후 채워서 리턴한다.
*/
TransformAnm* cASEParser::GetTransformAnm(const D3DXMATRIX& localTM)
{
	TransformAnm* pRscTransformAnm=NULL;
	vector<POSKEY*> arrayPOSKEY;
	vector<ROTKEY*> arrayROTKEY;
	vector<SCLKEY*> arraySCLKEY;

	pRscTransformAnm = m_SharedResourceMng.CreateTransformAnm();
