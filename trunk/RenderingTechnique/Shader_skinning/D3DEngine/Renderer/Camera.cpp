...
void cCamera::Render()
{
	...
	{
		m_matCamera=m_pAttachedModel->GetWorldTM();
//		MoveOnLocalAxis(0.0f,15.0f,9.0f);
	...
	{	
		...
		m_pD3DDevice9->SetVertexShaderConstantF( 8, (float*)temp, 4);
	...
	if ( m_bModifiedProjection)
	{
		...
		m_pD3DDevice9->SetVertexShaderConstantF( 12, (float*)temp, 4);
	}		
	...
	{	
		...
		m_pD3DDevice9->SetVertexShaderConstantF( 16, (float*)temp, 4);
...
void cCamera::Update(DWORD elapseTime)
{
	m_NumOutSide=0;
}
...
void cCamera::AttachToModel( cSceneRoot* pModel )
{
	assert(pModel!=NULL);
	if (m_pAttachedModel==NULL)
	{
