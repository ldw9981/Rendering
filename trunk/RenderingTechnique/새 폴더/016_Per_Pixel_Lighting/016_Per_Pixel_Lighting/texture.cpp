...
*/
int texture::load( char* filename, int repeat, int filter, unsigned int format)
{
	if(pt!=NULL)pt->Release();
...
//	pd->SetTextureStageState(0, D3DTSS_MAXANISOTROPY,16);
	...
	if( FAILED(
	D3DXCreateTextureFromFileEx( pd, filename, 0, 0, D3DX_DEFAULT, 0, (D3DFORMAT)format, D3DPOOL_DEFAULT, 
...
int texture::loadNoMessage(char *filename, int repeat, int filter, unsigned int format)
{
	if(pt!=NULL)pt->Release();
	if( FAILED(
	D3DXCreateTextureFromFileEx( pd, filename, 0, 0, D3DX_DEFAULT, 0, (D3DFORMAT)format, D3DPOOL_DEFAULT, 
...
/* 
 ...
 3 - trilinear, linear_mipmap_linear
*/
int texture::load( Image &im, int repeat, int filter, unsigned int format)
{
	if(im.data==NULL)return 0;
	int size;
	unsigned char* data;
	data = im.Get_dds( &size);
...
//	pd->SetTextureStageState(0, D3DTSS_MAXANISOTROPY,16);
	if(im.planes==1)format=D3DFMT_L8;
	if( FAILED(
	D3DXCreateTextureFromFileInMemoryEx( pd, data, size, 
						0, 0, D3DX_DEFAULT, 0, (D3DFORMAT)format, D3DPOOL_DEFAULT, 
						D3DX_FILTER_BOX, D3DX_FILTER_BOX, 0xFF000000, NULL, NULL, &pt)
				))
	{
		pt = NULL; 
