...
Scene::Scene()
{
	...
	sphere_map.load("data/dx_sky_box/spheremap.JPG");
	...
	if(!LoadVertexShader( "Shaders/009_sphere_mapping/vertex_shader.txt", dwDecl, &hVertexShader)){	error = 1;	return;}
}
...
void Scene::DrawDXScene()
{
	...
	if(keys['H'])				//vypise help
	{
		...
		if(english)
		{
			font0->Print(21,i++,"Program: sphere mapping", 0);
