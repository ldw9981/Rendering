...
int Image::Load(char *filename, int Message)
{
	char p[5];
	int ret = 0;
	...
	if(ret) return 1;
	if(Message)MessageBox( hWnd, filename, "Nenájdený súbor", MB_OK );
	return 0;
}
...
int Image::LoadTGA(char *filename)
{
	...
	byte flip;							// set on 1 if image must by flip
	...
	fread(&flip,   sizeof(byte), 1, pFile);
	if(flip&0x20)flip = 1;
	else flip = 0;
