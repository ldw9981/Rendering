...
// texture: 0 - no load, 1 - load base texture, 2 - load base and normal map texture 

int C3ds::Init(char *file_name, int filter, int faceNormal, int shadowObject, int texture, int compute_normals)
{
	z3ds_Chunk ch;
	...
	if(isShadowObject)
	{
		for(int i=0; i<objects.size(); i++)
		{
			if(objects[i].numOfFaces)
			{
				...
				if(objects[i].pFacesShadow==NULL)return 0;
			}
		}
		SetConnectivity();
	}
...
//	Rescale(3.0f);
	...
	if(texture)
		if(!LoadTextures( file_name, filter, texture==2))return 0;
	if(compute_normals)ComputeNormals();
	return 1;
}
	...
	for(int i = 0; i < objects.size(); i++)
	{
		...
		for(int j = 0; j < obj->numOfFaces; j++)
		{
			...
			for(int whichVertex = 0; whichVertex < 3; whichVertex++)
			{
				...
				{
					p[i_vec].t0.x = obj->pTexVerts[ index ].x;
					p[i_vec].t0.y = 1.f - obj->pTexVerts[ index ].y;
				}
				// vertex
				p[i_vec].v = obj->pVerts[ index ];
				i_vec++;
				ModifyBox(obj->pVerts[ index ]);
			}
		}
		dxobj.pvb->Unlock();
...
void C3ds::Render_dot3(int mode, DWORD hVertexShader)
{
	for(int i=0; i<dxobjs.size(); i++)
	{
		...
		if(mat!=NULL)
		{
			if(dxobjs[i].have_texture_coord_0 && textures!=NULL && textures[mat->textureNormalmapId].pt!=NULL)
			{
				switch(mode)
				{
				case 0:
...
void C3ds::Render_ambient(vec4 ambient)
{
	pd->SetRenderState( D3DRS_AMBIENT, D3DCOLOR_COLORVALUE(ambient.x,ambient.y,ambient.z,ambient.w) );
...
void C3ds::Render_dot3_specular(int mode, DWORD hVertexShader, int specExp, vec4 specCol)
{
	for(int i=0; i<dxobjs.size(); i++)
	{
		z3ds_MaterialInfo *mat=NULL;
		...
		if(mode==4 && (specCol.x<0.99f || specCol.y<0.99f || specCol.z<0.99f) )	// (N.H)^specExp x specCol
		{
			pd->SetRenderState( D3DRS_AMBIENT, D3DCOLOR_COLORVALUE(specCol.x,specCol.y,specCol.z,specCol.w) );
			pd->SetRenderState( D3DRS_LIGHTING, 1);
			...
			pd->SetRenderState( D3DRS_SRCBLEND, D3DBLEND_ZERO);
			pd->SetRenderState( D3DRS_DESTBLEND, D3DBLEND_SRCCOLOR );
			pd->SetRenderState( D3DRS_ZBIAS, 1);
...
void C3ds::Render_dot3_specular_cube_normalizer(int mode, DWORD hVertexShader, int specExp, vec4 specCol)
{
	...
	for(int i=0; i<dxobjs.size(); i++)
	{
		...
		if(mat!=NULL)
		{
			if(dxobjs[i].have_texture_coord_0 && textures[mat->textureNormalmapId].pt!=NULL)
			{
				...
				case 0:		// normal map
					...
					textures[mat->textureNormalmapId].SetTexture(1);
				...
				default:		// H.N
					...
					textures[mat->textureNormalmapId].SetTexture(1);
					pd->SetTextureStageState( 1, D3DTSS_COLOROP, D3DTOP_DOTPRODUCT3 );
					pd->SetTextureStageState( 1, D3DTSS_COLORARG1, D3DTA_TEXTURE );
					pd->SetTextureStageState( 1, D3DTSS_COLORARG2, D3DTA_CURRENT );
				}
				pd->SetTextureStageState( 0, D3DTSS_ALPHAOP,   D3DTOP_DISABLE );
		...
		if(mode==4 && (specCol.x<0.99f || specCol.y<0.99f || specCol.z<0.99f) )	// (N.H)^specExp x specCol
		{
			...
			pd->SetRenderState( D3DRS_ALPHABLENDENABLE, 0);
			pd->SetRenderState( D3DRS_LIGHTING, 0);
			pd->SetRenderState( D3DRS_AMBIENT, D3DCOLOR_COLORVALUE(0,0,0,0));
		}
		pd->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_DISABLE );
	}
	pd->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_DISABLE );
...
void C3ds::Render_pixel_shader(DWORD hVertexShader)
{
	for(int i=0; i<dxobjs.size(); i++)
	{
		z3ds_MaterialInfo *mat=NULL;
...
void C3ds::Transform(mat4 m)
{
	for(int i=0; i<objects.size(); i++)
	{
		...
		{
			for(int j=0; j<obj->numOfVerts; j++)
				obj->pVerts[j] = m*obj->pVerts[j];
		}
	}
}


void C3ds::ModifyBox(const vec &v)
{
