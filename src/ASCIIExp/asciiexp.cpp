...
#endif // !DESIGN_VER
		CheckDlgButton(hWnd, IDC_SKINNING_INFO,  exp->GetIncludeSkinningInfo()); 
...
#endif // !DESIGN_VER
			exp->SetIncludeSkinningInfo(IsDlgButtonChecked(hWnd, IDC_SKINNING_INFO));
...
BOOL AsciiExp::ReadConfig()
{
	...
	SetIncludeSkinningInfo(fgetc(cfgStream));
...
void AsciiExp::WriteConfig()
{
	...
	fputc(GetIncludeSkinningInfo(),		cfgStream);
