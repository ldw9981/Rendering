...
Font_bitmap::Font_bitmap(char *file_name)
{
	pvb = NULL;
	color.set(1,1,1,1);
...
void Font_bitmap::Print_vec(int stlpec, int riadok, const vec &a, char *meno)
{
	if(meno!=NULL)
	{
		Print(stlpec, riadok, meno,0);
		stlpec += strlen(meno)+1;
	}
	
	gcvt( (double)a.x, 2, text);
	Print(stlpec,riadok,text,0);
	stlpec+=10;
...
void Font_bitmap::Print_mat4(int stlpec, int riadok, const mat4 &m, char *nadpis)
{
	if(nadpis!=NULL)
	{
		Print(stlpec, riadok++, nadpis,0);
	}
	for(int i=0;i<16;i++)
	{
		...
		//  a02  a06  a10  a14
		//  a03  a07  a11  a15
		Print(stlpec+(i/4)*10,riadok+(i%4),text,0);
	}
}

void Font_bitmap::Print_float(int stlpec, int riadok, float x, char *meno)
{
	if(meno!=NULL)
	{
		Print(stlpec, riadok, meno,0);
...
void Font_bitmap::Print_int(int stlpec, int riadok, int a, char *meno)
{
	if(meno!=NULL)
	{
		Print(stlpec, riadok, meno,0);
