...
{	
	...
	{
		g_pD3DFramework->GetDebugInfoScene()->AddDebugString(L"cScene내부에서 ActiveCamera를 얻어 Render를 호출해야한다.\n");
	}
#endif
...
int cCameraNode::CheckWorldFrustum( cSphere* pSphere )
{
	assert(pSphere!=NULL);
	int ret;
	BOOL bInside=TRUE;
...
int cCameraNode::CheckWorldFrustum( cSphere* pSphere,WORD plane )
{
	int ret;
	BOOL bInside=TRUE;
...
*/
int cCameraNode::CheckWorldFrustumWithoutYAxis( cSphere* pSphere )
{
	assert(pSphere!=NULL);
	int ret;
	BOOL bInside=TRUE;
