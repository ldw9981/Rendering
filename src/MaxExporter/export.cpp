...
void AsciiExp::ExportGlobalInfo()
{
	...
	fprintf(pStream, "\t%s %d\n", ID_MESHFRAMESTEP, GetMeshFrameStep());
	fprintf(pStream, "\t%s %d\n", ID_KEYFRAMESTEP, GetKeyFrameStep());
...
void AsciiExp::ExportGeomObject(INode* node, int indentLevel)
{
	...
	{
		fprintf(pStream, "%s\t%s\n", indent.data(), ID_PROP_HIDDEN);
	}

	if(node->Renderable() == FALSE)
	{
		fprintf(pStream, "%s\t%s\n", indent.data(), ID_PROP_NORENDERABLE);
	}

	// 일반 Bone임을 확인하기!!!
	if(node->GetBoneNodeOnOff() == TRUE)
	{
		fprintf(pStream, "%s\t%s\n", indent.data(), ID_PROP_BONE);
	}
	// Biped로 Bone임을 확인하기!!!
	else
	{
		...
		// 밑에 코드로 Biped Controller가 있는지 없는지를 검사할 수 있다.
...
void AsciiExp::ExportMesh(INode* node, TimeValue t, int indentLevel)
{
	...
	Modifier*			modifierPhysique	 = NULL;
	ISkin*				skin				 = NULL;
	ISkinContextData*	skincontext			 = NULL;
	IPhysiqueExport*	physique			 = NULL;
	IPhyContextExport*	physiquecontext		 = NULL;

	LONG				NumberOfBone		 = 0;
	CHAR				BoneList[1024][256];
...
//	mesh->buildBoundingBox();
	...
	fprintf(pStream, "%s%s {\n",indent.data(),  ID_MESH);
	fprintf(pStream, "%s\t%s %d\n",indent.data(), ID_TIMEVALUE, t);
	if(GetIncludeSkinningInfo())
	{
		modifierPhysique = FindPhysiqueModifier(node);
		if(modifierPhysique != NULL)
		{
			physique	 = (IPhysiqueExport *)modifierPhysique->GetInterface(I_PHYEXPORT);
			if(physique != NULL)
			{
				physiquecontext = (IPhyContextExport *)physique->GetContextInterface(node);
				if(physiquecontext != NULL)
				{
					physiquecontext->ConvertToRigid(TRUE);
					physiquecontext->AllowBlending(TRUE);

					for (int i = 0; i<physiquecontext->GetNumberVertices(); i++)
					{
						IPhyVertexExport* vi = physiquecontext->GetVertexInterface(i);//, HIERARCHIAL_VERTEX);
						if (vi != NULL)
						{
							int type = vi->GetVertexType();
							switch (type)
							{
							case	RIGID_BLENDED_TYPE:		//we have a vertex assigned to more than one link
									{
										IPhyBlendedRigidVertex*		rb_vtx	 = (IPhyBlendedRigidVertex*)vi;
										for (int x=0; x<rb_vtx->GetNumberNodes(); x++)
										{
											TSTR		NodeName = rb_vtx->GetNode(x)->GetName();
											int			y;
											for(y=0; y<NumberOfBone; y++)
											{
												if(!strcmp(BoneList[y], NodeName))
												{
													break;
												}
											}
											...
											if(y == NumberOfBone)
											{
												strcpy(BoneList[NumberOfBone], NodeName);
												NumberOfBone++;
											}
										}
									}
							...
							case	RIGID_TYPE:		// we have a vertex assigned to a single link
									{
										...
										TSTR						NodeName = r_vtx->GetNode()->GetName();
										int							y;
										for(y=0; y<NumberOfBone; y++)
										{
											if(!strcmp(BoneList[y], NodeName))
					...
					fprintf(pStream, "%s\t%s %d\n",indent.data(), ID_MESH_NUMBONE, NumberOfBone);
					fprintf(pStream, "%s\t%s %d\n",indent.data(), ID_MESH_NUMSKINWEIGHT, physiquecontext->GetNumberVertices());
				}
			}
		}
		else
		{
			modifier		 = FindSkinModifier(node);
			if(modifier != NULL)
			{
				...
				if(skin != NULL)
				{
					...
					if(skincontext != NULL)
					{
						NumberOfBone		 = skin->GetNumBones();

						fprintf(pStream, "%s\t%s %d\n",indent.data(), ID_MESH_NUMBONE, NumberOfBone);
						fprintf(pStream, "%s\t%s %d\n",indent.data(), ID_MESH_NUMSKINWEIGHT, skincontext->GetNumPoints());
					}
				}
			}
		}
	...
	// Physique정보를 가지고 있으면 내보낸다.
	if(physiquecontext != NULL)
	{
		Matrix3	tempMatrix3;
//		skin->GetSkinInitTM(node, tempMatrix3);
		fprintf(pStream, "%s\t%s {\n",indent.data(), ID_SKIN_INITTM);
		fprintf(pStream, "%s\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW0, tempMatrix3.GetRow(0).x, tempMatrix3.GetRow(0).y, tempMatrix3.GetRow(0).z);
		fprintf(pStream, "%s\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW1, tempMatrix3.GetRow(1).x, tempMatrix3.GetRow(1).y, tempMatrix3.GetRow(1).z);
		fprintf(pStream, "%s\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW2, tempMatrix3.GetRow(2).x, tempMatrix3.GetRow(2).y, tempMatrix3.GetRow(2).z);
		fprintf(pStream, "%s\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW3, tempMatrix3.GetRow(3).x, tempMatrix3.GetRow(3).y, tempMatrix3.GetRow(3).z);
		fprintf(pStream, "%s\t}\n", indent.data());

		fprintf(pStream, "%s\t%s {\n",indent.data(), ID_BONE_LIST);
		for (i=0; i<NumberOfBone; i++)
		{
			fprintf(pStream, "%s\t\t%s %d {\n",indent.data(), ID_BONE, i);
			fprintf(pStream, "%s\t\t\t%s \"%s\"\n",indent.data(), ID_BONE_NAME, BoneList[i]);
			fprintf(pStream, "%s\t\t\t%s %s\n",indent.data(), ID_BONE_PROPERTY, "ABSOLUTE");

			Matrix3	tempMatrix3;
//			skin->GetBoneInitTM(tempNode, tempMatrix3);
			fprintf(pStream, "%s\t\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW0, tempMatrix3.GetRow(0).x, tempMatrix3.GetRow(0).y, tempMatrix3.GetRow(0).z);
			fprintf(pStream, "%s\t\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW1, tempMatrix3.GetRow(1).x, tempMatrix3.GetRow(1).y, tempMatrix3.GetRow(1).z);
			fprintf(pStream, "%s\t\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW2, tempMatrix3.GetRow(2).x, tempMatrix3.GetRow(2).y, tempMatrix3.GetRow(2).z);
			fprintf(pStream, "%s\t\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW3, tempMatrix3.GetRow(3).x, tempMatrix3.GetRow(3).y, tempMatrix3.GetRow(3).z);
		...
		int ver = physique->Version();


		fprintf(pStream, "%s\t%s {\n",indent.data(), ID_MESH_WVERTEXS);
		for (int i = 0; i<physiquecontext->GetNumberVertices(); i++)
		{
			...
			if (vi != NULL)
			{
				...
				case	RIGID_BLENDED_TYPE:		//we have a vertex assigned to more than one link
						{
							...
							for (int x=0; x<rb_vtx->GetNumberNodes(); x++)
							{
								...
								// 같은 Index가 있는지 찾아본다. 같은 Index가 있을 시에는 그냥 값을 더해주기만 하면 된다.
								int		z;
								for(z=0; z<tempNumberOfNode; z++)
								{
									if(tempIndex[z] == y)
									{
										tempValue[z]	+= rb_vtx->GetWeight(x);
										break;
									}
								}

								// 같은 Index를 못 찾았을 경우.
								if(z == tempNumberOfNode)
								{
									tempIndex[tempNumberOfNode]		 = y;
									tempValue[tempNumberOfNode]		 = rb_vtx->GetWeight(x);
									tempNumberOfNode++;
								}
							}

							for (int x=0; x<tempNumberOfNode; x++)
							{
								fprintf(pStream, "%s\t\t\t%s %d\t%f\n",indent.data(), ID_BONE_BLENGING_WEIGHT, tempIndex[x], tempValue[x]);
							}
						}
						break;
				...
				case	RIGID_TYPE:		// we have a vertex assigned to a single link
						{
							...
							TSTR				NodeName	 = r_vtx->GetNode()->GetName();
							for(int y=0; y<NumberOfBone; y++)
							{
								if(!strcmp(BoneList[y], NodeName))
								{
									fprintf(pStream, "%s\t\t\t%s %d\t%f\n",indent.data(), ID_BONE_BLENGING_WEIGHT, y, 1.0f);
									break;
								}
							}
						}
				...
				physiquecontext->ReleaseVertexInterface(vi);
		...
		physique->ReleaseContextInterface(physiquecontext);

		//Release the physique interface
		modifierPhysique->ReleaseInterface(I_PHYINTERFACE, physique);
	}
	// Bone정보를 가지고 있으면 내보낸다.
	else if(skincontext != NULL)
	{
		// Bone의 Index를 내보낸다.
		Matrix3	tempMatrix3;
		skin->GetSkinInitTM(node, tempMatrix3);
		...
		{
			fprintf(pStream, "%s\t\t%s %d {\n",indent.data(), ID_BONE, i);
			fprintf(pStream, "%s\t\t\t%s \"%s\"\n",indent.data(), ID_BONE_NAME, skin->GetBone(i)->GetName());
			...
			case	BONE_SPLINECLOSED_FLAG:
					boneProperty	 = "SPLINECLOSED";
					break;

			case	BONE_DRAW_ENVELOPE_FLAG:
					boneProperty	 = "DRAW_ENVELOPE";
					break;
			...
			default:
					boneProperty	 = "UNDEFINED";
					break;
			};

			fprintf(pStream, "%s\t\t\t%s %s\n",indent.data(), ID_BONE_PROPERTY, boneProperty);

			Matrix3	tempMatrix3;
			INode*	tempNode	 = skin->GetBone(i);
			skin->GetBoneInitTM(tempNode, tempMatrix3);
			fprintf(pStream, "%s\t\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW0, tempMatrix3.GetRow(0).x, tempMatrix3.GetRow(0).y, tempMatrix3.GetRow(0).z);
			fprintf(pStream, "%s\t\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW1, tempMatrix3.GetRow(1).x, tempMatrix3.GetRow(1).y, tempMatrix3.GetRow(1).z);
			fprintf(pStream, "%s\t\t\t%s %f\t%f\t%f\n", indent.data(), ID_TM_ROW2, tempMatrix3.GetRow(2).x, tempMatrix3.GetRow(2).y, tempMatrix3.GetRow(2).z);
		...
		// Export Weight
		// 각 Vertex의 Bone에 대한 Weight를 내보낸다.
		fprintf(pStream, "%s\t%s {\n",indent.data(), ID_MESH_WVERTEXS);
		for (i=0; i<skincontext->GetNumPoints(); i++)
		{
			fprintf(pStream, "%s\t\t%s %d {\n",indent.data(), ID_MESH_WEIGHT, i);
			for (int j=0; j<skincontext->GetNumAssignedBones(i); j++)
			{
				fprintf(pStream, "%s\t\t\t%s %d\t%f\n",indent.data(), ID_BONE_BLENGING_WEIGHT, skincontext->GetAssignedBone(i, j), skincontext->GetBoneWeight(i, j));
			}
			fprintf(pStream, "%s\t\t}\n",indent.data());
		}
		fprintf(pStream, "%s\t}\n",indent.data());
